...
macro_rules! ifs {
    ...
    // 第二种方法：批量映射
    {
        ...
        // * 📝实际上「在所有出现value的地方处理」就相当于「先得出value，然后处理，再返回」
        $(
            if (ifs!(@VALUE $f_guardian $guardian)) {
                ifs!(@VALUE $f_value $value)
            }
        )*
    };
    // 第四种方法：模式匹配
